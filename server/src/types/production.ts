export interface FabricRelaxationRecord {
  ...
  durationHours: number; // typically 24 hours for knit/fleece
...
export interface CartonPackingRecord {
  ...
  styleNumber: string;
  color: string;
  sizeRatio: Record<string, number>; // e.g. { S: 10, M: 20, L: 20, XL: 10 }
  totalPcsPerCarton: number;
  grossWeightKg: number;
  netWeightKg: number;
  cartonDimensionsCm: string; // e.g. '60 x 40 x 30'
  barcode: string;
  cbm: number;
  packedBy: string;
